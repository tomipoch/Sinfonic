...
export function SmartPlaylistDetailView() {
  ...
  if (!activeServerId) {
    return <p className="text-fg-subtle text-sm p-6">No server connected.</p>;
  }

  if (loading) {
    return <p className="text-fg-subtle text-sm p-6" role="status">Loading…</p>;
  }
  ...
  if (!playlist) {
    return (
      <div className="flex flex-col items-start gap-3 rounded-md border border-bg-raised bg-bg-subtle p-6 m-6">
        <div className="text-base font-medium text-fg">Smart playlist not found</div>
        <Link to="/smart-playlists" className="text-sm text-accent hover:underline">← Back to smart playlists</Link>
      </div>
    );
  }
  ...
  return (
    <section className="flex flex-col gap-6 p-6">
      <header className="flex flex-col gap-2">
        <Link to="/smart-playlists" className="text-xs text-fg-muted hover:text-fg">← Smart Playlists</Link>
      ...
      {tracks.length === 0 ? (
        <div className="flex flex-col items-start gap-3 rounded-md border border-bg-raised bg-bg-subtle p-6">
          <div className="text-base font-medium text-fg">No matching tracks</div>

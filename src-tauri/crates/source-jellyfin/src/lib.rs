...
//!
//! Phase 0 (Fase 0): skeleton with module structure. The HTTP client,
//! UDP discovery and auth flow are stubs that return
//! `ProviderError::Other("not implemented in skeleton")`. Real
...
/// Holds a connection session with a Jellyfin server.
#[derive(Clone, Debug)]
...
impl MusicProvider for JellyfinProvider {
    fn identity(&self) -> &Identity {
        unimplemented!("JellyfinProvider::identity — Phase 3")
    }

    fn capabilities(&self) -> &Capabilities {
        unimplemented!("JellyfinProvider::capabilities — Phase 3")
    }

    async fn home_sections(&self) -> ProviderResult<Vec<HomeSection>> {
        Err(ProviderError::Other("not implemented in skeleton".into()))
    }
